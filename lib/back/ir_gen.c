...
IRVal irgen_expr(TranslationUnit* tu, TB_Function* func, Expr* e) {
    switch (e->op) {
        ...
        case EXPR_CALL: {
            ...
            bool is_aggregate_return = !tu->target.arch->pass_return_via_reg(tu, e->type);
            size_t real_arg_count = is_aggregate_return ? 1 : 0;

            for (size_t i = 0; i < arg_count; i++) {
                Cuik_Type* arg_type = args[i]->type;
                real_arg_count += tu->target.arch->deduce_parameter_usage(tu, arg_type);
            }
            ...
            for (size_t i = 0; i < arg_count; i++) {
                ir_arg_count += tu->target.arch->pass_parameter(tu, func, args[i],
                    i >= varargs_cutoff,
        ...
        case EXPR_PRE_DEC: {
            ...
            if (type->kind == KIND_PTR) {
                ...
                TB_Register operation;
                if (is_inc)
                    operation = tb_inst_add(func, loaded, one, ab);
                else
                    operation = tb_inst_sub(func, loaded, one, ab);
        ...
        case EXPR_POST_DEC: {
            ...
            if (type->is_atomic) {
                ...
                TB_Register operation;
                if (is_inc)
                    operation = tb_inst_atomic_add(func, src.reg, stride, TB_MEM_ORDER_SEQ_CST);
                else
                    operation = tb_inst_atomic_sub(func, src.reg, stride, TB_MEM_ORDER_SEQ_CST);
        ...
        case EXPR_SHR: {
            ...
            if (type->kind == KIND_FLOAT || type->kind == KIND_DOUBLE) {
                switch (e->op) {
                    case EXPR_PLUS:
                    data = tb_inst_fadd(func, l, r);
                    break;
                    case EXPR_MINUS:
                    data = tb_inst_fsub(func, l, r);
                    break;
                    case EXPR_TIMES:
                    data = tb_inst_fmul(func, l, r);
                    break;
                    case EXPR_SLASH:
                    data = tb_inst_fdiv(func, l, r);
                    break;
                    default:
                    abort();
                }
                ...
                switch (e->op) {
                    case EXPR_PLUS:
                    data = tb_inst_add(func, l, r, ab);
                    break;
                    case EXPR_MINUS:
                    data = tb_inst_sub(func, l, r, ab);
                    break;
                    case EXPR_TIMES:
                    data = tb_inst_mul(func, l, r, ab);
                    break;
                    case EXPR_SLASH:
                    ...
                    break;
                    case EXPR_OR:
                    data = tb_inst_or(func, l, r);
                    break;
                    case EXPR_XOR:
                    data = tb_inst_xor(func, l, r);
                    break;
                    case EXPR_SHL:
                    data = tb_inst_shl(func, l, r, ab);
                    break;
                    case EXPR_SHR:
                    data = type->is_unsigned ? tb_inst_shr(func, l, r) : tb_inst_sar(func, l, r);
                    break;
                    default:
                    abort();
                }
        ...
        case EXPR_CMPLE: {
            ...
            if (type->kind == KIND_FLOAT || type->kind == KIND_DOUBLE) {
                switch (e->op) {
                    case EXPR_CMPGT:
                    data = tb_inst_cmp_fgt(func, l, r);
                    break;
                    case EXPR_CMPGE:
                    data = tb_inst_cmp_fge(func, l, r);
                    break;
                    case EXPR_CMPLT:
                    data = tb_inst_cmp_flt(func, l, r);
                    break;
                    case EXPR_CMPLE:
                    data = tb_inst_cmp_fle(func, l, r);
                    break;
                    default:
                    abort();
                }
                ...
                switch (e->op) {
                    case EXPR_CMPGT:
                    data = tb_inst_cmp_igt(func, l, r, false);
                    break;
                    case EXPR_CMPGE:
                    data = tb_inst_cmp_ige(func, l, r, false);
                    break;
                    case EXPR_CMPLT:
                    data = tb_inst_cmp_ilt(func, l, r, false);
                    break;
                    case EXPR_CMPLE:
                    data = tb_inst_cmp_ile(func, l, r, false);
                    break;
                    default:
                    abort();
                }
                ...
                switch (e->op) {
                    case EXPR_CMPGT:
                    data = tb_inst_cmp_igt(func, l, r, !type->is_unsigned);
                    break;
                    case EXPR_CMPGE:
                    data = tb_inst_cmp_ige(func, l, r, !type->is_unsigned);
                    break;
                    case EXPR_CMPLT:
                    data = tb_inst_cmp_ilt(func, l, r, !type->is_unsigned);
                    break;
                    case EXPR_CMPLE:
                    data = tb_inst_cmp_ile(func, l, r, !type->is_unsigned);
                    break;
                    default:
                    abort();
                }
