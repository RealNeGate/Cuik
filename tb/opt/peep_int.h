...
static bool is_iconst(TB_Function* f, TB_Node* n) { return lattice_is_iconst(latuni_get(f, n)); }
...
static Lattice* value_arith_raw(TB_Function* f, TB_NodeTypeEnum type, TB_DataType dt, Lattice* a, Lattice* b, bool congruent, bool at_top) {
    ...
    if (min == max) {
        Lattice* t = lattice_intern(f, (Lattice){ LATTICE_INT, ._int = { min, min, ~min, min } });

        if (congruent && type == TB_SUB) {
            if (at_top && lattice_meet(f, t, &FALSE_IN_THE_SKY) == t) {
...
static Lattice* value_shift(TB_Function* f, TB_Node* n) {
    ...
    if (b->tag == LATTICE_INT && b->_int.max > b->_int.min) {
        if (widen) {
            Lattice new_l = *lattice_from_dt(f, n->dt);
            new_l._int.widen = widen;
    ...
    if (b->_int.max <= bits) {
        ...
        switch (n->type) {
            ...
            case TB_SHR: {
                ...
                // if the number can be negative, just default to the full range
                if (min > max || ((max >> (bits - 1)) & 1)) {
                    min = lattice_int_min(bits);
                    max = lattice_int_max(bits);
                    zeros = ones = 0;
                }
...
static Lattice* value_bits(TB_Function* f, TB_Node* n) {
    ...
    // if the sign bit is known zero, we can trim the negative range
    if ((zeros >> (bits - 1)) & 1) {
        min = ones;
        max = TB_MIN(max, ~zeros);
    }
...
static TB_Node* identity_int_binop(TB_Function* f, TB_Node* n) {
    ...
    if (lattice_is_iconst(bb)) {
        ...
        if (lattice_int_eq(bb, 1)) {
            if (n->type == TB_MUL || n->type == TB_SDIV || n->type == TB_UDIV) {

            }
        } else if (lattice_int_eq(bb, 0)) {
            // a + 0 => a
            // a - 0 => a
            // a ^ 0 => a
            if (n->type == TB_ADD || n->type == TB_SUB || n->type == TB_XOR || n->type == TB_SHL || n->type == TB_SHR) {
                return n->inputs[1];
            }

            // ((a: bool) != 0) => a
            if (n->type == TB_CMP_NE && n->inputs[1]->dt.type == TB_TAG_BOOL) {
                return n->inputs[1];
            }
        }
    }
...
static TB_Node* ideal_extension(TB_Function* f, TB_Node* n) {
    ...
    if (ext_type == TB_SIGN_EXT) {
        ...
        int bits = tb_data_type_bit_size(NULL, src->dt.type);
    ...
    if (src->type == TB_ADD && lattice_is_iconst(value_of(f, src->inputs[2]))) {
        Lattice* lhs = latuni_get(f, src->inputs[1]);
        Lattice* rhs = latuni_get(f, src->inputs[2]);
