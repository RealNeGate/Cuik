...
static void construct_prologue_epilogue(Ctx* restrict ctx, TB_Function* f) {
    ...
    FOR_N(i, 0, ctx->param_count) {
        ...
        if (reg_num >= cc->param_count[reg_class]) {
            ...
            rm = intern_regmask2(ctx, REG_CLASS_GPR, false, cc->params[reg_class][reg_num]);
            used[reg_class] += 1;
        }
...
static void node_grammar_put(uint32_t k, uint32_t v) {
    ...
    uint32_t i = (k * 11400714819323198485llu) >> (64 - x86_grammar_exp);
    uint32_t first = i;
    uint32_t mask = (1u << x86_grammar_exp) - 1;
    do {
        if (x86_grammar[2*i + 0] == 0) {
            x86_grammar[2*i + 0] = k;
            x86_grammar[2*i + 1] = v;
            return;
        } else if (x86_grammar[2*i] == k) {
...
static uint32_t node_grammar_get(uint32_t k) {
    ...
    uint32_t mask = (1u << x86_grammar_exp) - 1;
    do {
        if (x86_grammar[2*i + 0] == k) {
            return x86_grammar[2*i + 1];
        } else if (x86_grammar[2*i + 0] == 0) {
            return 0;
        }
        i = (i + 1) & mask;
    } while (i != first);
