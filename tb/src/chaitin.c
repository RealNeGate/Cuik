...
static int ifg_remove_edges(Chaitin* ra, int* ws, int ws_cnt, uint32_t* visited, int i) {
    ...
    FOR_N(j, 0, ra->ifg_stride) {
        ...
        FOR_N(k, 0, 64) if ((bits >> k) & 1) {
            size_t id = j*64 + k;
            if ((visited[id / 32] & (1u << (id % 32))) == 0) {
                visited[id / 32] |= (1u << (id % 32));

                assert(ws_cnt < ra->ifg_len);
                ws[ws_cnt++] = id;
                TB_OPTDEBUG(REGALLOC)(printf("  Mark v%zu\n", id));
            }
...
static int simplify(Ctx* restrict ctx, Chaitin* restrict ra, int* stk, int cap) {
    ...
    // fill it up initially
    memset(visited, 0xFF, ((ra->ifg_len+31)/32) * sizeof(uint32_t));
    memset(visited_lo, 0, ((ra->ifg_len+31)/32) * sizeof(uint32_t));
    memset(visited_hi, 0, ((ra->ifg_len+31)/32) * sizeof(uint32_t));
    FOR_N(i, 1, ra->ifg_len) { ws[i - 1] = i; }
    ws_cnt = ra->ifg_len - 1;

    int cnt = 0;
    for (;;) {
        // placing any high degree nodes into the top of this array (temporarily)
        int hi_degree_pos = dyn_array_length(ra->spills);
        ...
        while (ws_cnt--) {
            ...
            if (vreg && (vreg->mask->class == REG_CLASS_STK || d < ctx->num_regs[vreg->mask->class])) {
                // don't wanna add twice but also we've got a lot of fucking visited sets now lol
                if ((visited_lo[vreg_id / 32] & (1u << (vreg_id % 32))) == 0) {
                    visited_lo[vreg_id / 32] |= (1u << (vreg_id % 32));
                    assert(cnt < cap);
                    stk[cnt++] = vreg_id;
                }

                // unmark as hi-degree
                if (visited_hi[vreg_id / 32] & (1u << (vreg_id % 32))) {
                    visited_hi[vreg_id / 32] &= ~(1u << (vreg_id % 32));
                    FOR_N(i, hi_degree_pos, dyn_array_length(ra->spills)) {
                ...
                // mark as hi-degree
                if ((visited_hi[vreg_id / 32] & (1u << (vreg_id % 32))) == 0) {
                    visited_hi[vreg_id / 32] |= (1u << (vreg_id % 32));
        ...
        if (dyn_array_length(ra->spills) == hi_degree_pos) {
            return cnt;
        }
        ...
        FOR_N(i, hi_degree_pos, dyn_array_length(ra->spills)) {
            ...
            if (vreg && ra->degree[i] >= ctx->num_regs[vreg->mask->class]) {
                float c = get_spill_cost(ctx, vreg);
                if (c < best_cost) {
                    TB_OPTDEBUG(REGALLOC)(printf("  better spill? v%d (%f is better than %f)\n", best_spill, c, best_cost));
                    best_cost  = c;
                    best_spill = i;
                }
            }
        ...
        assert(best_spill >= 0);
        TB_OPTDEBUG(REGALLOC)(printf("  found spill candidate v%d\n", best_spill));

        dyn_array_set_length(ra->spills, hi_degree_pos);
...
static void spill_entire_lifetime(Ctx* ctx, VReg* spill) {
    TB_Function* f = ctx->f;
    TB_Node* n = spill->n;
    ...
    for (;;) {
        ...
        CUIK_TIMED_BLOCK("hard splits") {
            ...
            FOR_N(i, 0, dyn_array_length(ra.spills)) {
                ...
                spill_entire_lifetime(ctx, &ctx->vregs[vreg_id]);
            }
        ...
        while (cnt) {
            ...
            TB_OPTDEBUG(REGALLOC)(printf("v%d:\n", vreg_id));
            TB_OPTDEBUG(REGALLOC)(printf("  => %#"PRIx64"\n", mask));
            ...
            FOR_N(j, 0, ra.ifg_stride) {
                ...
                FOR_N(k, 0, 64) if ((bits >> k) & 1) {
                    ...
                    if (fixed >= 0) {
                        mask &= ~(1ull << fixed);
                        TB_OPTDEBUG(REGALLOC)(printf("  => %#"PRIx64" (neighbor precolored R%d)\n", mask, fixed));
                    } else {
                        int assigned = other->assigned;
                        if (assigned >= 0) {
                            mask &= ~(1ull << assigned);
                            TB_OPTDEBUG(REGALLOC)(printf("  => %#"PRIx64" (we can't be R%d)\n", mask, assigned));
                        }
                    }
                }
            }
            ...
            if (mask == 0) {
                ...
                dyn_array_put(ra.spills, vreg_id);
                TB_OPTDEBUG(REGALLOC)(printf("  => FAIL\n"));
                continue;
            }

            vreg->class    = def_class;
            vreg->assigned = tb_ffs64(mask) - 1;
            TB_OPTDEBUG(REGALLOC)(printf("  => R%d\n", vreg->assigned));
        }
