...
TB_API bool tb_validator_next(TB_Function* f, TB_Validator* v) {
    CO_SCOPE(v) {
        ...
        for (; v->l < f->bb_count; v->l++) {
            v->end = f->bbs[v->l].end;
            if (v->end == NULL) continue; // Empty basic block

            v->n = f->bbs[v->l].start;
            while (v->n != 0 && v->n != v->end) {
                TB_Node* n = v->n;
                TB_FOR_INPUT_IN_NODE(in, n) {
                    if (v->n == *in) {
                        v->type = TB_VALIDATION_SELF_REFERENCE;
                        v->self_reference = v->n;
                        CO_YIELD(v, true);
                    }
                }

                if (TB_IS_NODE_TERMINATOR(v->n->type)) {
                    v->type = TB_VALIDATION_OUT_OF_PLACE_TERMINATOR;
                    v->out_of_place_terminator = v->n;

                    CO_YIELD(v, true);
                }

                TB_Node* next = v->n->next;
                if (next == NULL) {
                    v->type = TB_VALIDATION_NON_TERMINATOR_HAS_NO_NEXT;
                    v->non_terminator_no_next = v->n;
            ...
            if (!TB_IS_NODE_TERMINATOR(v->end->type)) {
