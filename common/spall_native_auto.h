...
#if SPALL_IS_X64
#include <x86intrin.h>
inline SPALL_FN uint64_t spall_get_clock(void) {
    return __rdtsc();
}
...
#if SPALL_IS_WINDOWS
inline SPALL_FN void spall_writer(void *arg) {
#else
inline SPALL_FN void *spall_writer(void *arg) {
#endif
...
SPALL_NOINSTRUMENT bool spall_auto_thread_init(uint32_t thread_id, size_t buffer_size) {
